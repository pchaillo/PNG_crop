...
RECORD_PDF = False # to record one by one all the cropped images in pdf format
...
nb_screen_per_image = 20

for i in liste : 
        for j in range(nb_screen_per_image) :
                u = source_folder + '/' + i
                crop_image = ci.crop_image(u)
                if RECORD_JPG :
                        record_path = destination_folder + '/jpg/' + i + '_' + str(j)
